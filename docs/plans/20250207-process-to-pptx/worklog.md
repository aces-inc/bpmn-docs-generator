...
## アクター名の四角・位置・矢印ラベル DoD（plan-execute 2025-02-11）

- **アクター名の四角**: アクター名の ROUNDED_RECTANGLE に `rect.line.color.rgb` と `rect.line.width = Pt(0.5)` を設定し、**枠線が表示され四角であることが視認できる**ようにした。
- **アクター名の位置**: 四角の top を「レーン中央 - box_height/2」で算出するように変更。**スイムレーン高さの縦方向中央**に四角とテキストが配置される。
- **分岐矢印のラベル**: 矢印中点の計算を `from_shp.left + from_shp.width` と `to_shp.left` で明示。ラベル用テキストボックスを約 2.5mm×1mm に拡大し、中点付近に配置。YAML で id が文字列のときのフォールバック lookup を追加。
- **process.yaml**: 成約? 分岐の `next` を `[{ id: 13, label: "見送り" }, { id: 14, label: "申込" }]` に変更し、矢印ラベルが PPTX に表示される例にした。
- **process-yaml SKILL**: 「レイアウトの考え方」に「四角には枠線が付き視認できる」を追記。YAML の記載方法は変更なし。
- plan.md の DoD: アクター名の位置・アクター名の四角・分岐矢印のラベル を [x] に更新。
- 品質ゲート: ruff / pytest 全通過。

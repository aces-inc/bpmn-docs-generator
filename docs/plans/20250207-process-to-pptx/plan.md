...
- **PPTX レイアウト仕様**
  ...
  - タスク同士は**矢印**（矢印付きの線）で繋ぐ。単なる線ではなく、先端が矢印になっていること。
  - 矢印の**接続ポイント**: タスクの四角の**上下中央**（縦方向の中心軸）の点に結合する。始点・終点とも四角の上下中央に合わせる。
  - **同一レーン内**のタスク間は直線矢印、**異なるレーン**のタスク間は **PPTX のデフォルトの折れ曲がり矢印**（直角に曲がるコネクタ / elbow connector）を使う。
...
- [x] **矢印の形状**: 接続線が矢印付き（先端が矢印）で描画されている（単なる線ではない）

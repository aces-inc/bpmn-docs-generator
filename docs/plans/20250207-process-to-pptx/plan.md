...
- **PPTX レイアウト仕様**
  ...
  - **アクター（スイムレーン）の名前**: 左側に記載する。**アクター名（およびそれを囲む四角）は、そのスイムレーンの高さの縦方向の中央**に配置する。スイムレーンの上下中央にない場合は不備とする。
  - **アクター名の表示形式**: アクター名は**点線から 2pt だけ離した長方形（四角）**の中に表示する。長方形は**角丸にせず角のある長方形**（RECTANGLE）とする。**塗りつぶしなし・枠線黒・フォント黒**とする。長方形の横幅は現状のアクター列の横幅に準拠する。四角内のテキストは**上下中央**で配置する。**アクター部分の点線の上下は 2pt ずつ離して四角があり、等間隔**になるようにする（点線―2pt―四角―2pt―点線の繰り返し）。**アクター枠に影は付けない**。
  - **アクター枠とタスク領域の間隔**: アクター名の四角の右端と、最初のタスク列の開始位置の間に**10pt の余白**を設ける。アクター枠と最初のタスクが引っ付かないようにする。
  ...
  - タスクは正方形の四角で表現する。**タスクのラベル（文字）は四角の外ではなく、図形（四角）の内側に配置する**。四角の余白はゼロ、文字は**黒**、**余白なし・折り返しなし**で、最小フォントは 10pt とする。**タスクの四角には影（shadow）を付けない**。**矢印・アクター枠・レーン線などすべての要素に影は付けない**。
  - **接続は矢印要素で描画する**: タスクから接続先タスクへのフローは、単なる線（Line）ではなく、**矢印の図形（コネクタ／矢印要素）**として描画する。接続先（終点）側に矢印の先端が付き、方向が分かるようにする。線だけの接続は不可。**矢印にも影は付けない**。
  - 矢印の**接続ポイント**（タスクのどの辺から出入りするか）:
    - **基本**: タスクから見て**左から入り、右から出る**。フローは左→右の列で進むため、送る側（始点）は**右辺中央**から出し、受け側（終点）は**左辺中央**で受ける。
    - **同列の場合のみ**: 送り元と送り先が同じ列（分岐先の縦並びやシステム接続など）のときは上下に伸ばす。上レーンへは上辺から出して上辺で受け、下レーンへは下辺から出して下辺で受ける。
  ...
  - **分岐矢印のラベル**: 分岐した際の矢印に、**何の説明か**を表すテキスト（例: Yes / No、承認 / 差し戻し）を付与できるようにする。YAML では `next` を **`[{ id: 先のID, label: "表示テキスト" }, ...]`** の形式で分岐先ごとにラベルを指定し、PPTX 上では**矢印の近くにそのテキストが表示される**こと。ラベルを指定した矢印にテキストが表示されない場合は不備とする。
...
- [x] **スライド左右10pt余白**: スイムレーン（アクター列＋タスク領域）がスライドの左端・右端からそれぞれ 10pt 以上離して配置されている。
- [x] **人のタスクの接続**: 人に属するタスク同士が必ず矢印で繋がっており、孤立した人のタスクが存在しない（YAML 検証または生成結果で確認可能）。
- [x] **アクター名の位置**: アクター名（およびその四角）が**スイムレーンの高さの縦方向中央**に配置されている。中央からずれている場合は不備とする。
- [x] **アクター名の四角**: アクター名が点線から 2pt 離した**長方形（角のある四角・角丸ではない）内**にあり、**塗りつぶしなし・枠線黒・フォント黒**。影なし。幅はアクター列幅に準拠し、テキストは上下中央。点線の上下 2pt ずつ離して四角があり等間隔になっている。
- [x] **アクター枠とタスクの間隔**: アクター枠の右端と最初のタスク列の間に 10pt の余白があり、引っ付いていない。
- [x] **スタート・ゴール**: 開始ノード・終了ノードが正円で描画されている。
- [x] **分岐図形**: 条件分岐が菱形＋✕、並行処理が菱形＋＋で描画されている。
- [ ] **分岐矢印のラベル**: YAML で `next: [{ id: 先のID, label: "表示テキスト" }, ...]` と指定した分岐矢印に、PPTX 上で説明テキスト（Yes/No 等）が**矢印の近くに表示されている**。ラベルを指定したのに表示されない場合は不備とする。
- [x] **ループ**: 分岐してスタートに戻るフローが YAML で定義でき、PPTX 上でループとして描画されている。
- [x] **成果物**: 成果物ノードがフローチャートのデータ図形で描画され、中に成果物名が記載されている。
...
- [ ] **矢印の接続点**: 矢印がタスクの辺に応じて正しく結合されている（前タスクから: 同レーンは左、上レーンは上、下レーンは下。次タスクへ: 同レーンまたは下は右、上は上）。矢印・アクター・レーン線に影が付いていない。

...
- **PPTX レイアウト仕様**
  ...
  - タスクは正方形の四角で表現する。余白はゼロの四角に、最小フォントは 10pt で文字を描く。
  - タスク同士は矢印で繋ぐ。矢印の距離（タスク間の間隔）は、正方形の一辺と同じ幅とする。
...
- **Docker ワークフロー**: 指定の **src** フォルダに変換前のファイル（YAML 等）を置き、`docker compose run` を実行すると変換が走り、変換後のファイル（PPTX 等）が **gen** フォルダに出力される。入力と出力の置き場所が明確であること。
...
- [x] **レイアウト**: アクター名は左、レーン間はグレー点線、タスクは正方形・余白ゼロ・最小 10pt、矢印間隔＝正方形一辺、改ページ時も同じ構造で続く
- [x] **矢印**: タスク同士の接続が PPTX 上で正しく矢印で繋がっている（接続子として描画される）
...
- [x] **Docker Compose**: 指定の **src** に変換前（YAML 等）を置き、`docker compose run` で変換し、変換後（PPTX 等）が **gen** フォルダに出力される `docker-compose` 設定があり、必要に応じてコンテナを増やせる状態である

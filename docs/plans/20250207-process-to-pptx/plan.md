...
- **PPTX レイアウト仕様**
  ...
  - 矢印の**接続ポイント**（タスクのどの辺から出入りするか）:
    - **前のタスクから受け取る側（終点）**: 同レーンから（左側から）の接続は**左辺中央**で受け、上レーンからは**上辺中央**で受ける。下レーンからは**下辺中央**で受ける。
    - **次のタスクへ送る側（始点）**: 次のタスクが同レーンまたは下のレーンのときは**右辺中央**から出し、次のタスクが上のレーンのときは**上辺中央**から出す。
...
- [x] **分岐矢印のラベル**: YAML で `next: [{ id: 先のID, label: "表示テキスト" }, ...]` と指定した分岐矢印に、PPTX 上で説明テキスト（Yes/No 等）が**矢印の近くに表示されている**。ラベルを指定したのに表示されない場合は不備とする。

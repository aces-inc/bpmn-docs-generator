...
- **スライド余白**  
  スイムレーン全体はスライドの左端・右端からそれぞれ 10pt 以上離して配置される。ルートの `layout.margins` で `left_pt` / `right_pt` / `top_pt` / `bottom_pt` を指定できる（未指定時は従来どおり）。

- **タスクサイズ・列数・フォント（layout）**  
  `layout.task_size_ratio` でタスクのレーン高さに対する比率（0.2～1.0、未指定時 0.6）を指定できる。`layout.max_cols_per_slide` で 1 スライドあたりの最大列数を指定できる（未指定時は幅から自動、9 列などに固定可能）。`layout.task_font_pt` / `actor_font_pt` / `label_font_pt` でフォントサイズ（pt）を指定できる（未指定時は 10 / 10 / 8）。アクターが 2～4 のときは描画領域の高さをレーンに割り当てて縦に広げ、タスクは指定サイズのまま中央配置される。スイムレーン区切り線（点線）はアクター四角の境界（四角と四角の間）に描画される。

...
| `request_to` | - | システム接続。このタスクからリクエストを送るサービスノードの ID のリスト。`[svc]` のほか、**`[{ id: svc, label: "リード登録" }]`** のようにオブジェクトのリストで書くと、矢印付近にアクション名が表示される。人タスク→サービスが点線（人側○・サービス側矢印）で描画される。 |
| `response_from` | - | システム接続。このタスクがレスポンスを受け取るサービスノードの ID のリスト。同様に `[{ id: svc, label: "結果取得" }]` でラベル付き指定可能。サービス→人タスクが下→上の点線で描画される。 |
...
- **ノードの形**  
  タスクはレーン高さに対する比率で決まる正方形（既定約 60%。`layout.task_size_ratio` で 0.2～1.0 を指定可能。同一列に複数ある場合は縦長の矩形）。**開始・終了**（`type: start` / `end`）は**正円**で描画される。分岐（`gateway`）はひし形で、条件分岐は **✕**、並行は **＋** が中に表示される。**成果物**（`type: artifact`）はフローチャートのデータ図形、**サービス**（`type: service`）は磁気ディスク図形で描画され、`label` に名前を書く。レーン間はグレーの点線で区切り、点線はレーン左端まで届く。

- **スライド余白・layout**  
  スイムレーン全体はスライドの左端・右端からそれぞれ 10pt 以上離して配置される。ルートの **`layout`** で余白・タスクサイズ・列数・フォントを指定する。`layout.margins` に `left_pt` / `right_pt` / `top_pt` / `bottom_pt` を指定すると、図の描画領域がその余白内に収まり、**レーン高さはその領域の高さをレーン数で割った値**になる（未指定時は従来どおり）。

- **タスクサイズ・列数・フォント（layout）**  
  ルートの `layout` で次を指定できる。`layout.task_size_ratio` でタスクのレーン高さに対する比率（0.2～1.0、未指定時 0.6）。`layout.max_cols_per_slide` で 1 スライドあたりの最大列数（未指定時は幅から自動）。`layout.task_font_pt` / `actor_font_pt` / `label_font_pt` でフォントサイズ pt（未指定時は 10 / 10 / 8）。**レーン高さ**は常に**マージン内の高さ（top_pt～bottom_pt の間）をレーン数で分割**した値となり、縦方向には縮小されない。幅が足りないときは横（タスク幅）のみ縮小され、指定した margin 内でレーンが均等に並ぶ。スイムレーン区切り線（点線）はアクター四角の境界に描画される。

- **システム用レーン**  
  アクター名に**接頭辞 `[システム]`** または**接尾辞 `_`** を付けると、そのアクターは 1 本の「システム」レーンに集約される。レーン内には `type: service` のノードの `label` から収集したユニークなシステム名が並んで配置される。システムへの矢印は**タスクの下辺**・**システム図形の上辺**で接続する。

...
- **layout**（任意）: レイアウトまわりの指定。**余白（マージン）**を変えたいときに使う。下記「余白（マージン）の指定」を参照。

---

## 余白（マージン）の指定

スライドの左右・上下の余白を変えたいときは、ルートに **`layout.margins`** を書く。縦横比の違うスライドに合わせて調整できる。

```yaml
layout:
  margins:
    left_pt: 10    # 左余白（pt）。未指定時は 10pt 以上を維持
    right_pt: 10   # 右余白（pt）
    top_pt: 0      # 上余白（pt）。未指定時はスライド高さの約 25% を確保
    bottom_pt: 0   # 下余白（pt）。未指定時は約 5%
actors:
  - 担当者
nodes:
  - id: 1
...
- **単位**: すべて **pt**（ポイント）。指定しないキーは従来どおりの既定値が使われる。
- **他者と共有する YAML** では、使った余白を `layout.margins` に書いておくと、誰が変換しても同じ見た目にしやすい。
...
- **スライド余白**  
  スイムレーン全体はスライドの左端・右端からそれぞれ 10pt 以上離して配置される。余白を変えたい場合はルートの **`layout.margins`** で指定する（上記「余白（マージン）の指定」参照）。
